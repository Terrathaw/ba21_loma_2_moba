...
public class PCMUtil {
    ...
    /**
     * <p>
     * Converts a {@code short} (signed, 16 bit) into a signed 64-bit {@code double}.
     * The float value is normalised and guaranteed to lie in the range [-1,1].
     * 16-bit PCM samples are stored as 2's-complement signed integers,
     * ranging from -32768 to 32767.
     * </p>
     ...
     */
    public static double short2Double(short sample) {
        return (double) sample * (1.0d / FULL_SCALE);
    }
    ...
    /**
     * <p>
     * Converts an array of {@code double} into an array of signed 16-bit {@code short}.
     * </p>
     *
     * @param samples an array of {@code double}
     * @return an array of {@code short}
     */
    public static short[] double2ShortArray(double[] samples) {
        short[] output = new short[samples.length];
        for (int i = 0; i < output.length; i++) {
            double out = samples[i] * FULL_SCALE;
            if (out < Short.MIN_VALUE) {
    ...
    public static void fillSampleChannels(float[] samples, float[][] channels) {
        int nrChannels = channels.length;
        for (int i = 0; i < channels[0].length; i += nrChannels) {
            for (int j = 0; j < nrChannels; j++) {
                channels[j][i] = samples[i + j];
            }
        }
    }

    public static float[] flattenSampleChannels(float[][] channels) {
        int nrChannels = channels.length;
        float[] samples = new float[channels[0].length * nrChannels];
        for (int i = 0; i < channels[0].length; i += nrChannels) {
            for (int j = 0; j < nrChannels; j++) {
                samples[i + j] = channels[j][i];
            }
        }
        return samples;
    }
